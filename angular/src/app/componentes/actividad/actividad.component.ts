      ...
      switchMap((resultados: any[]) => {
        ...
        resultados.forEach(({ seguido, seguimientos, opiniones }) => {
          ...
          const actOpin = opiniones.map((o: any) => ({
            ...
            idSerie: o.idSerie,
            fecha: o.fecha,
            descripcion: o.opinion || '', 
            titulo: 'Ha publicado una reseña'
          }));
        ...
        return forkJoin(peticionesSeries).pipe(
          map(seriesDetalles => {
            ...
            return actividadCompuesta
              .map(item => ({
                ...item,
                serie: mapaSeries.get(Number(item.idSerie)) || mapaSeries.get(item.idSerie) // fix ID type
              }))
